...
struct RootView: View {
 ...
 var body:some View{ZStack{PaperTexture();GeometryReader{g in let W:CGFloat=390,H:CGFloat=780;let s=min(g.size.width/W,g.size.height/H);VStack(spacing:0){header.frame(height:52);hero.frame(height:248);rule;stats.frame(height:180);rule;metrics.frame(height:62);action.frame(height:72);rule;nav.frame(height:52);rule;log.frame(height:48)}.padding(.horizontal,24).frame(width:W,height:H,alignment:.top).scaleEffect(s,anchor:.top).frame(width:g.size.width,height:g.size.height,alignment:.top)}}.preferredColorScheme(.light)
 .sheet(isPresented:$showLogin){LoginWebView(onCookie:{app.saveCookie($0);showLogin=false},onDiagnostic:{app.diagnostic($0)})}.sheet(isPresented:$showFullLog){FullLogView().environmentObject(app)}.sheet(isPresented:$showCharacterStats){CharacterStatsView().environmentObject(app)}.sheet(isPresented:$showDailyQuests){DailyQuestsView().environmentObject(app)}.onChange(of:scenePhase){_,p in app.handleScenePhase(p)}.task{await app.refreshCharacterProfile()}}
 ...
 VStack(spacing:4){rule;HStack{Text(app.activity?.localizedTitle ?? "Selecione");Spacer();Text("\(app.stats.successes)/\(app.activity == nil ? app.goal:app.sessionGoal)")}.font(.system(size:18));beads;HStack{Text("Progresso \(app.stats.successes)/\(app.activity == nil ? app.goal:app.sessionGoal)");Spacer();Text(app.activity == nil ? "Aguardando resultado":app.state.label)}.font(.system(size:10.5)).lineLimit(1);activityTray}.foregroundStyle(ink)}}
 private var beads:some View{HStack(spacing:5){ForEach(0..<12,id:\.self){i in Circle().fill(i<max(0,min(12,Int(ceil(app.progress*12)))) ? copper:paper).frame(width:15,height:15).overlay(Circle().stroke(ink.opacity(0.20),lineWidth:1)).shadow(color:.black.opacity(0.13),radius:1,x:1,y:1)}}.padding(5).background(Capsule().fill(paper).shadow(color:.white.opacity(0.95),radius:2,x:-2,y:-2).shadow(color:.black.opacity(0.18),radius:3,x:2,y:2))}
 private var activityTray:some View{ZStack{ActivityPanelShape().fill(paper).shadow(color:.white.opacity(0.95),radius:2,x:-2,y:-2).shadow(color:.black.opacity(0.20),radius:3,x:2,y:2).overlay(ActivityPanelShape().stroke(ink.opacity(0.10)));VStack(spacing:4){HStack(spacing:0){tile(.tree);tile(.coal);tile(.iron);tile(.silver);tile(.cacti)};HStack(spacing:0){tile(.fishing);tile(.chicken);tile(.zombie);tile(.dragon);Color.clear.frame(maxWidth:.infinity)}}.padding(.horizontal,5).padding(.vertical,9)}}
 ...
 private var metrics:some View{HStack(spacing:0){metric("Meta da sessão",app.activity==nil ? "\(app.goal)":"\(app.sessionGoal)");vline;metric("Sucessos","\(app.stats.successes)");vline;metric("Falhas","\(app.stats.failures)");vline;metric("Tentativas","\(app.stats.attempts)")}.padding(.vertical,6)}
...
private struct ActivityPanelShape:Shape{func path(in r:CGRect)->Path{var p=Path();let cr:CGFloat=17,top:CGFloat=30,mid=r.height*0.52,w=r.width/5;p.move(to:CGPoint(x:cr,y:0));p.addLine(to:CGPoint(x:r.width-cr,y:0));p.addQuadCurve(to:CGPoint(x:r.width,y:cr),control:CGPoint(x:r.width,y:0));p.addLine(to:CGPoint(x:r.width,y:r.height-cr));p.addQuadCurve(to:CGPoint(x:r.width-cr,y:r.height),control:CGPoint(x:r.width,y:r.height));p.addLine(to:CGPoint(x:cr,y:r.height));p.addQuadCurve(to:CGPoint(x:0,y:r.height-cr),control:CGPoint(x:0,y:r.height));p.addLine(to:CGPoint(x:0,y:mid+10));for i in 0..<5{let x=CGFloat(i)*w;p.addLine(to:CGPoint(x:x+w*0.5,y:mid-12));p.addLine(to:CGPoint(x:x+w,y:mid+10))};p.addLine(to:CGPoint(x:r.width,y:cr));p.closeSubpath();return p}}
