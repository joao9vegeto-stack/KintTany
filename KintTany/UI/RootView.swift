...
struct RootView: View {
 ...
 var body:some View{ZStack{PaperTexture();GeometryReader{g in let W:CGFloat=390,H:CGFloat=844;let s=min(g.size.width/W,g.size.height/H);VStack(spacing:0){header.frame(height:58);hero.frame(height:258);rule;stats.frame(height:190);rule;metrics.frame(height:66);action.frame(height:78);rule;nav.frame(height:58);rule;log.frame(height:56)}.padding(.horizontal,22).frame(width:W,height:H,alignment:.top).scaleEffect(s,anchor:.top).frame(width:g.size.width,height:g.size.height,alignment:.top)}}.preferredColorScheme(.light)
 ...
 private var header:some View{ZStack{Text("KintTany").font(.system(size:28,weight:.regular));HStack(spacing:8){Circle().fill(app.connected ? Color(red:0.38,green:0.48,blue:0.36):Color(red:0.68,green:0.52,blue:0.29)).frame(width:14,height:14).overlay(Circle().stroke(ink.opacity(0.55)));Text(app.connected ? "ONLINE":"PRONTO").font(.system(size:14));Spacer()}}.foregroundStyle(ink)}
 private var hero:some View{HStack(spacing:14){ZStack{if app.hasSession{CharacterVoxel3DView(appearance:app.characterProfile.appearance).scaleEffect(1.52).padding(-24)}else{Image(systemName:"person.crop.square").font(.system(size:46)).foregroundStyle(ink.opacity(0.35))}}.frame(width:108).embossedPanel(cornerRadius:14,fill:paper).onTapGesture{if app.hasSession{showCharacterStats=true}else{showLogin=true}}
 ...
 private var activityTray:some View{ZStack{RoundedRectangle(cornerRadius:17).fill(paper).shadow(color:.white.opacity(0.95),radius:2,x:-2,y:-2).shadow(color:.black.opacity(0.20),radius:3,x:2,y:2).overlay(RoundedRectangle(cornerRadius:17).stroke(ink.opacity(0.10)));VStack(spacing:4){HStack(spacing:0){tile(.tree);tile(.coal);tile(.iron);tile(.silver);tile(.cacti)};ReliefDivider().stroke(ink.opacity(0.12),lineWidth:1).frame(height:13);HStack(spacing:0){tile(.fishing);tile(.chicken);tile(.zombie);tile(.dragon);Color.clear.frame(maxWidth:.infinity)}}.padding(.horizontal,5).padding(.vertical,7)}}
 @ViewBuilder private func tile(_ m:ActivityMode)->some View{Button{guard app.activity==nil else{return};app.start(m)}label:{VStack(spacing:2){Image(sprite(m)).resizable().scaledToFit().frame(width:37,height:37).shadow(color:.black.opacity(0.22),radius:1,x:1,y:1);Text(m.localizedTitle).font(.system(size:9.5)).lineLimit(1).minimumScaleFactor(0.72)}.foregroundStyle(ink).frame(maxWidth:.infinity,maxHeight:.infinity)}.buttonStyle(.plain).disabled(app.activity != nil)}
 ...
 private var stats:some View{VStack(alignment:.leading,spacing:4){Text("STATS").font(.system(size:20));HStack(spacing:10){VStack(spacing:4){ForEach(CharacterSkill.allCases){skillRow($0)};rule;HStack(spacing:4){Text("Total Level").frame(width:64,alignment:.leading);bar(Double(app.characterProfile.totalLevel)/40).frame(width:118);Text("\(app.characterProfile.totalLevel)").frame(width:30,alignment:.trailing)}.font(.system(size:11.5))}.frame(width:220);Rectangle().fill(ink.opacity(0.28)).frame(width:1);VStack(alignment:.leading,spacing:5){Image(systemName:"mountain.2.fill").font(.system(size:29)).foregroundStyle(Color(red:0.38,green:0.43,blue:0.34));datum("Região",app.world.serverRegion ?? app.player.region);datum("Posição",String(format:"%0.1f, %0.1f",app.player.position.x,app.player.position.z));datum("Recursos","\(app.resourceCount)");datum("Mobs","\(app.mobCount)");Text("Último evento").font(.system(size:11));Text(app.stats.lastEvent.isEmpty ? "—":app.stats.lastEvent).font(.system(size:10.5)).lineLimit(2)}.frame(maxWidth:.infinity,alignment:.leading)}}.foregroundStyle(ink).padding(.horizontal,6).padding(.vertical,5)}
 private func skillRow(_ s:CharacterSkill)->some View{let l=app.characterProfile.skills.level(for:s);return HStack(spacing:4){Text(skillName(s)).frame(width:64,alignment:.leading);bar(Double(l)/40).frame(width:118);Text("\(l)/40").frame(width:30,alignment:.trailing).lineLimit(1)}.font(.system(size:11.5))}
