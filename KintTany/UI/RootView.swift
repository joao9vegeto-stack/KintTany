...
struct RootView: View {
 ...
 private let ink=Color(red:0.12,green:0.115,blue:0.105), copper=Color(red:0.66,green:0.34,blue:0.23), paper=Color(red:0.92,green:0.91,blue:0.88)
 var body:some View{ZStack{PaperTexture();GeometryReader{g in let W:CGFloat=390,H:CGFloat=760;let s=min(g.size.width/W,g.size.height/H);VStack(spacing:0){header.frame(height:50);hero.frame(height:238);rule;stats.frame(height:176);rule;metrics.frame(height:60);action.frame(height:70);rule;nav.frame(height:50);rule;log.frame(height:46)}.padding(.horizontal,24).frame(width:W,height:H,alignment:.top).scaleEffect(s,anchor:.top).frame(width:g.size.width,height:g.size.height,alignment:.top)}}.preferredColorScheme(.light)
 .sheet(isPresented:$showLogin){LoginWebView(onCookie:{app.saveCookie($0);showLogin=false},onDiagnostic:{app.diagnostic($0)})}.sheet(isPresented:$showFullLog){FullLogView().environmentObject(app)}.sheet(isPresented:$showCharacterStats){CharacterStatsView().environmentObject(app)}.sheet(isPresented:$showDailyQuests){DailyQuestsView().environmentObject(app)}.onChange(of:scenePhase){_,p in app.handleScenePhase(p)}.task{await app.refreshCharacterProfile()}.alert("Meta da sessão",isPresented:$showGoalEditor){TextField("1–100000",text:$goalDraft).keyboardType(.numberPad);Button("Cancelar",role:.cancel){};Button("Salvar"){if let value=Int(goalDraft){app.goal=min(100_000,max(1,value))}}}message:{Text("Digite qualquer meta entre 1 e 100000.")}}
 private var rule:some View{Rectangle().fill(ink.opacity(0.42)).frame(height:1)}
 private var header:some View{ZStack{Text("KintTany").font(.system(size:26,weight:.regular));HStack(spacing:8){Circle().fill(app.connected ? Color(red:0.38,green:0.48,blue:0.36):Color(red:0.68,green:0.52,blue:0.29)).frame(width:13,height:13).overlay(Circle().stroke(ink.opacity(0.55)));Text(app.connected ? "ONLINE":"PRONTO").font(.system(size:13));Spacer()}}.foregroundStyle(ink)}
 private var hero:some View{HStack(spacing:13){ZStack{if app.hasSession{CharacterVoxel3DView(appearance:app.characterProfile.appearance).scaleEffect(1.52).padding(-24)}else{Image(systemName:"person.crop.square").font(.system(size:46)).foregroundStyle(ink.opacity(0.35))}}.frame(width:106).embossedPanel(cornerRadius:14,fill:paper).onTapGesture{if app.hasSession{showCharacterStats=true}else{showLogin=true}}
 VStack(spacing:4){rule;HStack{Text(app.activity?.localizedTitle ?? "Selecione");Spacer();Text("\(app.stats.successes)/\(app.activity == nil ? app.goal:app.sessionGoal)")}.font(.system(size:18));beads;HStack{Text("Progresso \(app.stats.successes)/\(app.activity == nil ? app.goal:app.sessionGoal)");Spacer();VStack(alignment:.trailing,spacing:0){Text(app.activity == nil ? "Aguardando resultado":app.state.label);TimelineView(.periodic(from:.now,by:1)){ctx in Text(app.activity == nil ? "0.00/min":app.formattedRatePerMinute(at:ctx.date)).font(.system(size:9.5))}}}.font(.system(size:10.5)).lineLimit(1);activityTray}.foregroundStyle(ink)}}
 private var beadGoal:Int{max(1,app.activity == nil ? app.goal:app.sessionGoal)}
 private var filledBeads:Int{guard app.stats.successes > 0 else{return 0};return min(10,Int(floor(Double(app.stats.successes)*10.0/Double(beadGoal))))}
 private var beads:some View{HStack(spacing:6){ForEach(0..<10,id:\.self){i in Circle().fill(i<filledBeads ? copper:paper).frame(width:16,height:16).overlay(Circle().stroke(ink.opacity(0.20),lineWidth:1)).shadow(color:.black.opacity(0.13),radius:1,x:1,y:1)}}.padding(5).background(Capsule().fill(paper).shadow(color:.white.opacity(0.95),radius:2,x:-2,y:-2).shadow(color:.black.opacity(0.18),radius:3,x:2,y:2))}
 private var activityTray:some View{VStack(spacing:0){activityRow([.tree,.coal,.stone,.iron,.silver]);activityRow([.cacti,.fishing,.chicken,.zombie,.dragon])}.background(RoundedRectangle(cornerRadius:17).fill(paper).shadow(color:.white.opacity(0.95),radius:2,x:-2,y:-2).shadow(color:.black.opacity(0.22),radius:3,x:2,y:2)).overlay(RoundedRectangle(cornerRadius:17).stroke(ink.opacity(0.10),lineWidth:1)).clipShape(RoundedRectangle(cornerRadius:17))}
 private func activityRow(_ modes:[ActivityMode])->some View{ZStack{ScallopRowShape(teeth:modes.count).fill(paper).shadow(color:.black.opacity(0.13),radius:2,x:0,y:-1);HStack(spacing:0){ForEach(modes,id:\.self){tile($0)}}.padding(.horizontal,4).padding(.top,12).padding(.bottom,4)}}
 @ViewBuilder private func tile(_ m:ActivityMode)->some View{Button{guard app.activity==nil else{return};app.start(m)}label:{VStack(spacing:2){Image(sprite(m)).resizable().scaledToFit().frame(width:39,height:39).shadow(color:.black.opacity(0.22),radius:1,x:1,y:1);Text(m.localizedTitle).font(.system(size:9.5)).lineLimit(1).minimumScaleFactor(0.72)}.foregroundStyle(ink).frame(maxWidth:.infinity,maxHeight:.infinity)}.buttonStyle(.plain).disabled(app.activity != nil)}
 ...
 private var stats:some View{VStack(alignment:.leading,spacing:4){Text("STATS").font(.system(size:21));HStack(spacing:8){VStack(spacing:4){ForEach(CharacterSkill.allCases){skillRow($0)};rule;HStack(spacing:4){Text("Total Level").frame(width:60,alignment:.leading);bar(Double(app.characterProfile.totalLevel)/40).frame(width:126);Text("\(app.characterProfile.totalLevel)").frame(width:42,alignment:.trailing)}.font(.system(size:11.2))}.frame(width:236);Rectangle().fill(ink.opacity(0.28)).frame(width:1);VStack(alignment:.leading,spacing:5){Image(systemName:"mountain.2.fill").font(.system(size:29)).foregroundStyle(Color(red:0.38,green:0.43,blue:0.34));datum("Região",app.world.serverRegion ?? app.player.region);datum("Posição",String(format:"%0.1f, %0.1f",app.player.position.x,app.player.position.z));datum("Recursos","\(app.resourceCount)");datum("Mobs","\(app.mobCount)");Text("Último evento").font(.system(size:11));Text(app.stats.lastEvent.isEmpty ? "—":app.stats.lastEvent).font(.system(size:10.5)).lineLimit(2)}.frame(maxWidth:.infinity,alignment:.leading)}}.foregroundStyle(ink).padding(.horizontal,6).padding(.vertical,5)}
 private func skillRow(_ s:CharacterSkill)->some View{let l=app.characterProfile.skills.level(for:s);return HStack(spacing:4){Text(skillName(s)).frame(width:60,alignment:.leading);bar(Double(l)/40).frame(width:126);Text("\(l)/40").frame(width:42,alignment:.trailing).lineLimit(1).fixedSize(horizontal:true,vertical:false)}.font(.system(size:11.5))}
 ...
 private func bar(_ p:Double)->some View{GeometryReader{g in ZStack(alignment:.leading){Capsule().fill(Color.black.opacity(0.08)).overlay(Capsule().stroke(Color.black.opacity(0.10))).shadow(color:.white.opacity(0.8),radius:1,x:-1,y:-1).shadow(color:.black.opacity(0.16),radius:1,x:1,y:1);Capsule().fill(copper).frame(width:max(5,g.size.width*min(1,max(0,p))))}}.frame(height:11)}
 private func datum(_ a:String,_ b:String)->some View{HStack(spacing:3){Text(a);Text(b).lineLimit(1).minimumScaleFactor(0.65)}.font(.system(size:10.5))}
 private var metrics:some View{HStack(spacing:0){Button{guard app.activity==nil else{return};goalDraft="\(app.goal)";showGoalEditor=true}label:{metric("Meta da sessão",app.activity==nil ? "\(app.goal)":"\(app.sessionGoal)")}.buttonStyle(.plain).disabled(app.activity != nil);vline;metric("Sucessos","\(app.stats.successes)");vline;metric("Falhas","\(app.stats.failures)");vline;metric("Tentativas","\(app.stats.attempts)")}.padding(.vertical,6)}
 private var vline:some View{Rectangle().fill(ink.opacity(0.30)).frame(width:1,height:44)}
 private func metric(_ t:String,_ v:String)->some View{VStack(spacing:2){Text(t).font(.system(size:10));Text(v).font(.system(size:22))}.frame(maxWidth:.infinity).foregroundStyle(ink)}
 private var action:some View{Button{if app.activity != nil{app.stop()}}label:{HStack(spacing:16){Image(systemName:"stop.fill");Text(app.activity == nil ? "PRONTO":"PARAR")}.font(.system(size:20)).foregroundStyle(Color(red:0.95,green:0.92,blue:0.87)).frame(maxWidth:.infinity,minHeight:56,maxHeight:56).background(RoundedRectangle(cornerRadius:14).fill(app.activity == nil ? copper.opacity(0.25):copper).shadow(color:.white.opacity(0.9),radius:2,x:-2,y:-2).shadow(color:.black.opacity(0.28),radius:3,x:2,y:2)).overlay(RoundedRectangle(cornerRadius:14).stroke(ink.opacity(0.58)))}.buttonStyle(.plain).disabled(app.activity==nil)}
 private var nav:some View{HStack(spacing:0){navB("STATS","chart.bar.fill"){showCharacterStats=true};vnav;navB("QUESTS","list.bullet"){showDailyQuests=true};vnav;navB("SESSÃO","slider.horizontal.3"){showLogin=true}}}
 private var vnav:some View{Rectangle().fill(ink.opacity(0.30)).frame(width:1,height:31)}
 private func navB(_ t:String,_ i:String,_ a:@escaping()->Void)->some View{Button(action:a){HStack(spacing:8){Image(systemName:i).font(.system(size:18));Text(t).font(.system(size:12.5))}.foregroundStyle(ink).frame(maxWidth:.infinity,maxHeight:.infinity)}.buttonStyle(.plain)}
 private var log:some View{Button{showFullLog=true}label:{HStack{Image(systemName:"list.bullet.rectangle");Text("Log completo");Spacer();Image(systemName:"chevron.right")}.font(.system(size:13.5)).foregroundStyle(ink).padding(.horizontal,8)}.buttonStyle(.plain)}
}
private struct ScallopRowShape:Shape{let teeth:Int;func path(in r:CGRect)->Path{var p=Path();let n=max(1,teeth),w=r.width/CGFloat(n),d:CGFloat=14;p.move(to:CGPoint(x:0,y:d));for i in 0..<n{let x=CGFloat(i)*w;p.addLine(to:CGPoint(x:x+w*0.5,y:0));p.addLine(to:CGPoint(x:x+w,y:d))};p.addLine(to:CGPoint(x:r.width,y:r.height));p.addLine(to:CGPoint(x:0,y:r.height));p.closeSubpath();return p}}
private struct ReliefDivider:Shape{func path(in r:CGRect)->Path{var p=Path();let n=5;let w=r.width/CGFloat(n);p.move(to:CGPoint(x:0,y:r.height));for i in 0..<n{let x=CGFloat(i)*w;p.addLine(to:CGPoint(x:x+w*0.5,y:0));p.addLine(to:CGPoint(x:x+w,y:r.height))};return p}}
private struct PaperTexture:View{var body:some View{ZStack{Color(red:0.92,green:0.91,blue:0.88);Canvas{c,s in for i in 0..<650{let x=CGFloat((i*73)%997)/997*s.width,y=CGFloat((i*149)%991)/991*s.height;c.fill(Path(ellipseIn:CGRect(x:x,y:y,width:0.65,height:0.65)),with:.color(.black.opacity(0.022)))}}}.ignoresSafeArea()}}
private struct EmbossedPanelModifier:ViewModifier{let cornerRadius:CGFloat;let fill:Color;func body(content:Content)->some View{content.background(RoundedRectangle(cornerRadius:cornerRadius).fill(fill).shadow(color:.white.opacity(0.92),radius:2,x:-2,y:-2).shadow(color:.black.opacity(0.19),radius:3,x:2,y:2)).overlay(RoundedRectangle(cornerRadius:cornerRadius).stroke(Color.black.opacity(0.10),lineWidth:1))}}
